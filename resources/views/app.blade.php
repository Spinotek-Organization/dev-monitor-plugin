...
<head>
    ...
    <title>Spinotek Dev Monitor & Ticket</title>
    <!-- Tailwind CSS CDN -->
...
<body class="bg-slate-50 text-slate-900 min-h-screen antialiased">
    <div id="app" v-cloak class="min-h-screen flex flex-col md:flex-row bg-slate-50">
        ...
        <div v-if="isMobileSidebarOpen" @click="isMobileSidebarOpen = false" class="fixed inset-0 z-40 bg-slate-900/40 backdrop-blur-xs md:hidden"></div>

        <!-- ================= SIDEBAR ================= -->
        <aside class="fixed md:sticky top-0 left-0 z-40 h-screen w-64 bg-white border-r border-slate-200 flex flex-col flex-shrink-0 transition-transform duration-300 ease-in-out md:translate-x-0 shadow-sm md:shadow-none"
               :class="isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'">
            <!-- Brand Logo Header -->
            <div class="h-16 flex items-center justify-between px-5 border-b border-slate-100">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center p-1.5 shadow-2xs flex-shrink-0">
                        <img src="{{ route('spinotek.ticket.logo') }}" alt="Spinotek Logo" class="w-full h-full object-contain">
                    </div>
                    <div>
                        <span class="font-bold text-base text-slate-900 tracking-tight block leading-tight">Spinotek</span>
                        <span class="text-[10px] font-semibold text-blue-600 tracking-wider uppercase">Dev Monitor</span>
                    </div>
                </div>
                <!-- Close Button on Mobile -->
                <button @click="isMobileSidebarOpen = false" class="md:hidden p-1.5 text-slate-400 hover:text-slate-600 rounded-lg transition cursor-pointer">
            ...
            <!-- Navigation Links -->
            <div class="flex-1 px-3.5 py-6 space-y-6 overflow-y-auto">
                <div>
                    <p class="px-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Main Menu</p>
                    <nav class="space-y-1.5">
                        <!-- Task Monitoring Menu Item -->
                        <button @click="switchTab('tasks'); isMobileSidebarOpen = false"
                                class="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-medium transition group cursor-pointer text-left"
                                :class="activeTab === 'tasks' ? 'bg-blue-50 text-blue-700 font-semibold border border-blue-200/70 shadow-2xs' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'">
                            <div class="flex items-center gap-3">
                                <svg class="w-5 h-5 flex-shrink-0 transition" :class="activeTab === 'tasks' ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-600'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                                <span>Task Monitoring</span>
                            </div>
                            <span class="text-xs px-2 py-0.5 rounded-full font-bold"
                                  :class="activeTab === 'tasks' ? 'bg-blue-200/70 text-blue-800' : 'bg-slate-100 text-slate-600'">
                                @{{ computedStats.total }}
                            </span>
                        </button>

                        <!-- Version Logs Menu Item -->
                        <button @click="switchTab('version-logs'); isMobileSidebarOpen = false"
                                class="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-medium transition group cursor-pointer text-left"
                                :class="activeTab === 'version-logs' ? 'bg-blue-50 text-blue-700 font-semibold border border-blue-200/70 shadow-2xs' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'">
                            <div class="flex items-center gap-3">
                                <svg class="w-5 h-5 flex-shrink-0 transition" :class="activeTab === 'version-logs' ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-600'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            ...
                            <span class="text-[11px] px-2 py-0.5 rounded-md font-mono font-bold"
                                  :class="activeTab === 'version-logs' ? 'bg-blue-200/70 text-blue-800' : 'bg-slate-100 text-slate-600'">
            ...
            <div class="p-4 border-t border-slate-100 bg-slate-50/50">
                <div class="bg-white rounded-xl p-3 border border-slate-200/80 shadow-2xs">
                    <div class="flex items-center gap-2">
                        <span class="relative flex h-2 w-2">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                        </span>
                        <span class="text-xs font-semibold text-slate-700">Plugin Active</span>
                    </div>
                    <p class="text-[11px] text-slate-400 mt-1">Spinotek Ticket &bull; Vue 3 SPA</p>
                </div>
            </div>
        </aside>
        ...
        <div class="flex-1 min-w-0 flex flex-col min-h-screen">
            <!-- Top Bar Header -->
            <header class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-20 shadow-2xs">
                <div class="flex items-center gap-3">
                    <!-- Hamburger Toggle on Mobile -->
                    <button @click="isMobileSidebarOpen = !isMobileSidebarOpen" class="md:hidden p-2 text-slate-600 hover:text-slate-900 rounded-xl hover:bg-slate-100 transition cursor-pointer">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
                        </svg>
                    </button>
                    <div class="flex items-center gap-2 text-xs sm:text-sm text-slate-500">
                        <span class="font-medium text-slate-700">Spinotek</span>
                        <span>/</span>
                        <span class="font-bold text-slate-900">@{{ activeTab === 'tasks' ? 'Task Monitoring' : 'Version Logs' }}</span>
                    </div>
                </div>

                <!-- Top Bar Action Button -->
                <div class="flex items-center gap-3">
                    <button v-if="activeTab === 'tasks'" @click="openCreateModal" class="inline-flex items-center gap-2 px-3.5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-semibold rounded-xl shadow-xs shadow-blue-200 transition cursor-pointer">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                        </svg>
                        <span>Tambah Task</span>
                    </button>
                    <button v-else-if="activeTab === 'version-logs'" @click="openAddVersionModal" class="inline-flex items-center gap-2 px-3.5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-semibold rounded-xl shadow-xs shadow-blue-200 transition cursor-pointer">
                        <svg class="w-4 h-4 flex-shrink-0" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                        </svg>
                        <span>Catat Versi</span>
                    </button>
                </div>
            </header>
            ...
            <!-- Footer -->
            <footer class="bg-white border-t border-slate-200 py-4 px-6 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-2 mt-auto">
                <div class="flex items-center gap-2">
                ...
                <p class="text-slate-400">Vue 3 Single Page Application</p>
            </footer>
        </div>
